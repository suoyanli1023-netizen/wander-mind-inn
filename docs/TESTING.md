...
**当前实际执行：** 2026-09-19，在 Windows、Chrome 153、静态 HTTP 服务与全新测试 profile 中，对 `bfeffb0` 执行 05A 稳定性最终验收。本文第 16 节起记录本轮证据。04C-5、04C-6 与 04D 是 2026-09-17 的历史实际执行记录。
...
04C-5、04C-6 和 04D 是 2026-09-17 的历史实际执行记录；04C-4 及更早阶段同样保留为历史验收记录。04D 本地最终整合验收结论为 **FAIL=0、BLOCKED=0**。当前 05A 实测证据见后续章节。AI Gateway、Safety Guard、真实服务端认证、数据库、生产环境部署和真实 AI 内容效果测试尚未执行，不能从当前结果推断其质量或上线状态。

## 16. 05A 稳定性修复与最终验收（当前实际执行）

提交链为 `a1d021d fix-house-base-value-mapping` → `13406c4 fix-atmosphere-state-persistence` → `d8b4edd fix-atmosphere-lifecycle-cleanup` → `bfeffb0 fix-atmosphere-mobile-default-placement`。本轮从空 localStorage 和干净浏览器 profile 实际完成演示登录、旅程新建／重命名、五模块完整流程、同一旅程五类贴纸、贴纸删除、刷新、旅程删除取消／确认、音效关闭／开启与刷新，最终重新创建旅程后仍可继续使用。

五类贴纸实际为 `情绪小屋·平静`、`氛围小屋·情绪空间`、`绘画投射`、`释放·漂流瓶`、`云朵·情绪需求`。极简小屋的 `classic`、`treehouse`、`igloo` 均生成对应 SVG，并命中各自安慰语与专业解读，不再落入错误默认分支。

## 17. 氛围状态、兼容性和生命周期

- `soul_journey` 正式字段为 `archives`、`houseState`、`atmosphereState`、`drawState`、`worryState`、`choiceState`。
- 旧数据无 `atmosphereState` 时采用默认空状态；部分字段可恢复；`furniture` / `characters` 非数组时降级为空数组；混入的异常记录被过滤，合法记录保留并转为数值字段；其他五个业务字段均保持原值。
- 背景、基底、家具、人物、位置和尺寸在退出、刷新、再次进入后与 localStorage 一致；连续进入三次未重复生成 DOM。
- `done` 实际转换为 `false → true → false → true`：完成结果后写入 `true`，有效编辑后写回 `false`，再次完成恢复为 `true`。
- 添加 5 个动态元素后，删除其中 2 个会移除对应元素监听；连续进入／退出 10 次，每次退出后的临时交互注册表、document 动态监听、画布临时监听和动画 timer 均为 0。对同一元素拖拽 20 次，监听器数量前后完全一致。
- 连续完成 3 次均只产生一个结果区；每轮完成后 timer 集合归零。在五阶段动画第 1、2、3、4 阶段分别退出并等待超过完整动画时长，均无迟到结果、无迟到音效，临时监听器和 timer 为 0；再次进入仍可编辑并正常完成。

Mouse 拖拽、Touch 拖拽、`touchcancel`、四角缩放、Wheel 0.5～2 倍边界、双指缩放、删除、Canvas 图片处理、平均色采样、保存贴纸及刷新恢复均实际执行。音频调用包装在分阶段测试中重复安装造成测试数组重复记录，但每个阶段退出前后的数组严格不变；这是测试计数器现象，不是产品重复播放。

## 18. 默认位置与七视口

实际在每个视口添加 13 件家具、5 个摆件、4 个人物，再添加一轮 13 件家具以覆盖模板循环，共 35 个动态元素。修复前审计确认家具／摆件共用 8 个位置模板，人物使用 4 个模板，`x` / `y` 是左上角百分比，`w` 为像素宽度；右侧第 2、8 个模板在窄画布存在水平风险，高纵横比 PNG 还会产生垂直裁切。修复调整右侧模板，并仅在新图片加载后按实际元素尺寸夹取坐标；恢复持久化场景不经过该适配。

| 实际视口 | 氛围画布宽×高 | 默认元素越界数 / 35 | `clientWidth / scrollWidth` | 横向溢出 |
|---|---:|---:|---:|---:|
| 320×844 | 256×192 | 0 | 320 / 320 | 0 |
| 360×844 | 296×222 | 0 | 360 / 360 | 0 |
| 390×844 | 326×244.5 | 0 | 390 / 390 | 0 |
| 414×844 | 350×262.5 | 0 | 414 / 414 | 0 |
| 480×844 | 416×312 | 0 | 480 / 480 | 0 |
| 768×844 | 616×462 | 0 | 762 / 762 | 0 |
| 1366×768 | 402×301.5 | 0 | 1360 / 1360 | 0 |

每个视口还实际遍历登录、首页、旅程、五模块、结果区、设置面板和退出弹窗。关键按钮可操作，页面与画布无水平滚动；320px 下全部默认元素完整位于画布内。768／1366px 保持桌面布局。竖向滚动条导致后两行 `clientWidth` 比 `innerWidth` 少 6px，不属于横向溢出。

## 19. 05A XSS、资源与运行时

三个 payload 均通过真实放走坏心情输入流程完成写入、存储、DOM 显示和刷新：

| Payload | script 前／后／刷新后 | `img[onerror]` | alert | STATE / localStorage / DOM | 双重转义 |
|---|---:|---:|---:|---|---|
| `<script>alert(1)</script>` | 15 / 15 / 15 | 0 / 0 / 0 | 0 / 0 / 0 | 均保留原始文本，DOM 为普通文字 | 否 |
| `<img src=x onerror=alert(1)>` | 15 / 15 / 15 | 0 / 0 / 0 | 0 / 0 / 0 | 均保留原始文本，DOM 为普通文字 | 否 |
| `javascript:alert(1)` | 15 / 15 / 15 | 0 / 0 / 0 | 0 / 0 / 0 | 均保留原始文本，DOM 为普通文字 | 否 |

资源清单实际核对 13 个 JS、11 个 CSS、14 个 WAV、42 个 PNG、根目录 favicon 与 `index.html`，共 82 个请求，全部 HTTP 200。`favicon.svg` 为 `image/svg+xml`，没有 `/favicon.ico` 请求。干净产品会话统计：HTTP 4xx/5xx=0、网络加载失败=0、`console.error=0`、`pageerror=0`、`unhandledrejection=0`。

测试使用浏览器内 `prompt` / `confirm`、alert 计数和音频自动播放替身；Mouse、Touch、Wheel、页面点击、输入、刷新和目标视口均通过真实浏览器事件执行。兼容性测试中的虚构坏资源路径曾产生测试工具 404，已隔离并用真实资源清单重测，不计入产品错误。

**05A 最终产品验收：FAIL=0，BLOCKED=0。** 所有临时脚本、profile、报告和服务器均在系统临时目录并于验收后清理。真实 AI、Agent、Safety Guard、服务端 Gateway、数据库、生产认证和生产部署测试尚未执行。
