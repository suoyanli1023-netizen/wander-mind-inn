...
function buildHouseInterpretation() {
  ...
  const baseNames = { classic: '经典小屋', tree: '树屋', dome: '圆顶屋' };
  ...
  const baseInsights = {
    ...
    tree: `你选了"树屋"——既在树上，又看得见远方。这个选择很有意思：树屋是一种"双重位置"——被树枝包裹（庇护），但又在高处俯瞰（眺望）。你此刻的内心，可能正处在一种矛盾中：一方面想躲起来、不被打扰，另一方面又不想完全切断与世界的联系。你选择了一个"既在又不在"的位置——既在世界上，又保持一点距离。这不是逃避，而是一种有分寸的自我保护：我需要空间，但我还没准备好完全关上门。`,
    dome: `你选了"圆顶屋"——没有尖角，没有棱角，一切都是圆的。圆顶在空间心理学中对应着"包容"和"完整"。选择圆顶的人，此刻内心可能正在拒绝"被切割"的感觉——也许你最近经历了太多非此即彼、左右为难的处境，你厌倦了对立和冲突。圆顶说的是："我不想再被逼着选边站了，我想要一个能容纳所有面的空间。"你的内心在寻求一种整体性的接纳——好和坏、强和弱，都能在同一个圆里共存。`
  };
  const bgInsights = {
  ...
  // 综合洞察：把所有选择串起来看
  const comboParts = [];
  if (base === 'tree' && bg === 'night') comboParts.push('你想待在高处看星空——内心渴望一种"有距离感的辽阔"，也许最近的日常让你觉得太挤了');
